...
module.exports = {
  ...
  async execute(client, interaction, args) {
    ...
    if (interaction.guild.me.voice?.channel && interaction.member.voice.channel.id !== interaction.guild.me.voice.channel.id)
      return interaction.reply({ content: `:no_entry_sign: You must be listening in **${interaction.guild.me.voice.channel.name}** to use that!`, ephemeral:true  }).catch((err) => {
    ...
    if (!songName)
      return interaction.reply({
        content: ":no_entry_sign: **Please include a query.**", ephemeral:true   }).catch((err) => {
        console.log(`i couldn't reply to the message: ` + err.message)
                });
    interaction.reply({
      content: `:watch: Searching ... (\`${songName}\`)` }).then(async (m) => {
      try {
...
async function playMusic(index, player, m, res, queue, interaction) {
  try {
    ...
    queue.destroy();
    return await interaction.reply({ content: "**Couldn't join your voice channel!**", ephemeral:true }).catch((err) => {console.log(`i couldn't reply to the message: ` + err.message)
    })
  }
